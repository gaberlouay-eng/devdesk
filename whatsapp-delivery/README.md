...
- `review.js` — שלב 5: ממשק CLI לצפייה, עריכה ואישור טיוטות.

## שלב 5 — אישור טיוטות (CLI)

`review.js` מציג את כל הטיוטות הממתינות (`status='draft'`) בטרמינל ומאפשר
לאשר (מקצה `doc_number` ורושם `confirmed_at`) או לערוך שדה לפני האישור.
שום דבר לא מאושר אוטומטית - האישור הוא תמיד פעולה ידנית.
...
- `c <id>` — אישור הזמנה (מקצה מספר תעודה).
- `e <id> <שדה>=<ערך>` — עריכת שדה (`customer_name`, `material`, `quantity`,
  `destination`, `date`, `notes`) לפני האישור.
- `l` — רענון הרשימה.
- `q` — יציאה.

## מה הלאה

PDF יטופל מחדש בנפרד (ראו הערה למעלה על שלב 2) - הכיוון המוצע הוא HTML->PDF
דרך דפדפן headless במקום pdfmake/pdfkit.

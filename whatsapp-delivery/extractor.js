...
function buildSystemPrompt(knownEntities) {
  ...
  const destinationsList = (knownEntities.destinations || []).join(', ') || '(אין)';
...
- רשימת יעדים/ערים ידועים (בהודעה עלול להופיע שם היעד בערבית או בתעתיק - נסה להתאים לשם העברי הרשמי): ${destinationsList}
- אם מילה בהודעה נשמעת דומה פונטית או קרובה בכתיב לשם ברשימה (לקוח, חומר או יעד), החזר את הצורה הרשמית מהרשימה, לא את הכתיב המקורי או התעתיק מההודעה.
- אם השם/החומר/היעד בהודעה לא דומה לשום פריט ברשימה, החזר את מה שנאמר כפי שהוא (אל תכריח התאמה לרשימה, ואל תתרגם באופן חופשי - רק אם יש התאמה סבירה לרשימה).
